...
class _LoraAdamHelper:
  ...
  def get_unmagnified_rotate_first_moment(
      self, m, basis_other_old, basis_other, dim, dim_other
  ):
    m, shape = self.move_lora_dim_to_last(m, dim)
    basis_other_old, _ = self.move_lora_dim_to_last(basis_other_old, dim_other)
    basis_other, _ = self.move_lora_dim_to_last(basis_other, dim_other)

    p = basis_other.T @ basis_other_old

    old_norm = torch.linalg.norm(m, axis=1, keepdims=True)
    m = m @ p.T
    new_norm = torch.linalg.norm(m, axis=1, keepdims=True)
    # new_norm should be smaller than old_norm
    #m = m*torch.minimum(old_norm/new_norm, torch.ones_like(old_norm))
    m = torch.nan_to_num(m)
    return self.restore_original_shape_and_dim(m, dim, shape)

  def get_unmagnified_rotate_second_moment(
      self, v, basis_other_old, basis_other, dim, dim_other
  ):
    basis_other_old, _ = self.move_lora_dim_to_last(basis_other_old, dim_other)
    basis_other, _ = self.move_lora_dim_to_last(basis_other, dim_other)
  ...
  def precond_unmagnified_rotate_sqrt_dim(
      self,
      g,
      p,
      c,
...
class LoRARite(Optimizer):
  def __init__(
    ...
    self.extra_lr_scale = extra_lr_scale
    self.extra_optimizer = None
    if extra_params is not None:
        self.extra_optimizer = torch.optim.AdamW(extra_params, lr=lr, betas=(beta1, beta2), eps=epsilon, weight_decay=weight_decay)
    ...
    self.helper = _LoraAdamHelper()
    self.subtract_subspace = subtract_subspace
    self.state_initialized = False
    self.count = 0
    print("apply_c", self.apply_c)
    print("subtract_subspace", self.subtract_subspace)
  ...
  def step(self, closure=None):
    ...
    if self.count%10 == 0:
      param_norm_sq = 0
      extra_param_norm_sq = 0
      extra_param_grad_norm_sq = 0
      param_norms = []
      extra_param_norms = []
      for group in self.param_groups:
        for p in group["params"]:
          param_norm_sq += torch.linalg.norm(p.data)**2
          param_norms.append(float(torch.linalg.norm(p.data)))
      param_norm = torch.sqrt(param_norm_sq)
      if self.extra_optimizer is not None:
        for group in self.extra_optimizer.param_groups:
          for p in group["params"]:
            extra_param_norm_sq += torch.linalg.norm(p.data)**2
            extra_param_grad_norm_sq += torch.linalg.norm(p.grad.data)**2
            extra_param_norms.append(float(torch.linalg.norm(p.data)))
      #extra_param_norm = torch.sqrt(extra_param_norm_sq)
      #extra_param_grad_norm = torch.sqrt(extra_param_grad_norm_sq)
      #print("param_norm", param_norm, "extra_param_norm", extra_param_norm)
      #print("param_norms", param_norms)
      #print("extra_param_norms", extra_param_norms)
      #print("extra_param_grad_norm", extra_param_grad_norm)
    self.count += 1
    ...
    for group in self.param_groups:
      ...
      for p1, p2 in list(zip(group["params"], group["params"][1:]))[::2]:
        ...
        transformed_v_l = helper.get_unmagnified_rotate_second_moment(
            state.v_l,
            state.basis_r_old,
            basis_r,
            lora_l_dim,
        ...
        transformed_v_r = helper.get_unmagnified_rotate_second_moment(
            state.v_r,
            state.basis_l_old,
            basis_l,
            lora_r_dim,
        ...
        f = helper.precond_unmagnified_rotate_sqrt_dim
        updates_l = f(
            updates_l,
            v_l,
            c_l,
        ...
        m_l = helper.get_unmagnified_rotate_first_moment(
            state.m_l,
            state.basis_r_old,
            basis_r,
            lora_l_dim,
        ...
        m_r = helper.get_unmagnified_rotate_first_moment(
            state.m_r,
            state.basis_l_old,
            basis_l,
            lora_r_dim,
        ...
        if align_param:
          params_l, params_r = p1.data.to(torch.float32), p2.data.to(torch.float32)
          p_l = helper.move_lora_dim_to_last(params_l, lora_l_dim)[0]
          p_r = helper.move_lora_dim_to_last(params_r, lora_r_dim)[0]

          q_l, r_l = torch.linalg.qr(p_l)
          q_r, r_r = torch.linalg.qr(p_r)

          U, S, Vh = torch.linalg.svd(r_l@r_r.T, full_matrices=False)
          S = torch.unsqueeze(S, 0)

          # TODO Need to decide which side base on init
          p_l = q_l@U
          p_r = q_r@Vh.T*S

          params_l = helper.restore_param_shape(p_l, params_l, lora_l_dim)
          params_r = helper.restore_param_shape(p_r, params_r, lora_r_dim)

          p1.data.copy_(params_l.data)
          p2.data.copy_(params_r.data)
...
class LoRAScale(Optimizer):
  def __init__(
      ...
      lora_l_dim: int = 0,
      lora_r_dim: int = -1,
      lora_l_name: str = 'w_prime_left',
      lora_r_name: str = 'w_prime_right',
      maybe_inf_to_nan: bool = True,
      balance_param: bool = True,
      extra_params = None,
      extra_lr_scale = 1,
  ):
    print("LoRAScale init")
    beta1, beta2 = betas
    epsilon = eps
    epsilon_root = 0 # TODO
    if lr < 0.0:
  ...
  def step(self, closure=None):
    ...
    if not self.state_initialized:
      ...
      for group in self.param_groups:
        ...
        for p1, p2 in list(zip(group["params"], group["params"][1:]))[::2]:
          ...
          state = self.state[p1]["attr"]
          state.step = 0
          state.v_l = torch.zeros_like(params_l)
          state.v_r = torch.zeros_like(params_r)
          state.m_l = torch.zeros_like(params_l)
          state.m_r = torch.zeros_like(params_r)
    ...
    for group in self.param_groups:
      ...
      for p1, p2 in list(zip(group["params"], group["params"][1:]))[::2]:
        ...
        updates_l, updates_r = helper.inf_to_nan(p1.grad.data).to(torch.float32), helper.inf_to_nan(p2.grad.data).to(torch.float32)
        updates_l = torch.nan_to_num(updates_l/(torch.linalg.norm(params_r)/torch.sqrt(torch.tensor(params_r.shape[lora_r_dim]))))
        updates_r = torch.nan_to_num(updates_r/(torch.linalg.norm(params_l)/torch.sqrt(torch.tensor(params_l.shape[lora_l_dim]))))
      ...
      for p1, p2 in list(zip(group["params"], group["params"][1:]))[::2]:
        ...
        if clip_unmagnified_grad > 0:
          if g_norm > clip_unmagnified_grad:
            updates_l = (
                updates_l
                / g_norm.to(updates_l.dtype)
                * clip_unmagnified_grad
            )
            updates_r = (
                updates_r
                / g_norm.to(updates_r.dtype)
                * clip_unmagnified_grad
            )

        m_l, v_l = helper.update_moments(count, updates_l, state.m_l, state.v_l, beta1, beta2)
        m_r, v_r = helper.update_moments(count, updates_r, state.m_r, state.v_r, beta1, beta2)

        updates_l = m_l / (torch.sqrt(v_l + epsilon_root) + epsilon)
        updates_r = m_r / (torch.sqrt(v_r + epsilon_root) + epsilon)
    ...
    if self.extra_optimizer is not None:
      lr = self.param_groups[0]["lr"]*self.extra_lr_scale
...
class LoRADiag(Optimizer):
  def __init__(
    ...
    print("scale_update", self.scale_update)
  ...
  def step(self, closure=None):
    ...
    for group in self.param_groups:
      ...
      for p1, p2 in list(zip(group["params"], group["params"][1:]))[::2]:
        ...
        updates_l = helper.move_lora_dim_to_last(updates_l, lora_l_dim)[0]
        updates_r = helper.move_lora_dim_to_last(updates_r, lora_r_dim)[0]

        p_l = helper.move_lora_dim_to_last(params_l, lora_l_dim)[0]
        p_r = helper.move_lora_dim_to_last(params_r, lora_r_dim)[0]

        updates_l = torch.nan_to_num(updates_l/torch.linalg.norm(p_r, axis=0))
        updates_r = torch.nan_to_num(updates_r/torch.linalg.norm(p_l, axis=0))

        updates_l = helper.restore_param_shape(updates_l, params_l, lora_l_dim)
        updates_r = helper.restore_param_shape(updates_r, params_r, lora_r_dim)
    ...
    for group in self.param_groups:
      ...
      for p1, p2 in list(zip(group["params"], group["params"][1:]))[::2]:
        ...
        if self.scale_update:
          scale = torch.linalg.norm(p_l, axis=0)*torch.linalg.norm(p_r, axis=0)
          scale = torch.maximum(scale, torch.ones_like(scale))
          updates_l = updates_l*scale
          updates_r = updates_r*scale
